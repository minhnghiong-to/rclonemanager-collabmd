...
prepare_docker_volume_dirs() {
  ...
  collabmd_git_secrets_dir="$(resolve_host_path "${COLLABMD_GIT_SECRETS_DIR:-$volume_root/collabmd/git/secrets}")"
...
if [ "${COLLABMD_LOCAL_ENABLED:-true}" = "true" ]; then
  PROFILE_ARGS+=(--profile collabmd-local)
fi
